...
contract Documents {
  ...
  function addDocument (bytes32 docHash, bytes assigneeHash, address assignee, string docName, string extension) {
  ...
  function getDocumentByHash(bytes32 docHash) constant public returns(address issuer, address assignee , bytes assigneeHash, uint _id, uint timestamp, string name, string extension) {
    Document doc = documents[docHash];
    issuer  = doc.issuer;
  ...
  function getDocumentById(uint docID) constant public returns(address issuer, address assignee, bytes assigneeHash ,uint _id, uint timestamp, string name, string extension) {
